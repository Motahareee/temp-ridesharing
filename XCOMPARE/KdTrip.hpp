...
public:
  struct Trip {
    ...
    bool operator < (const KdTrip::Trip& v) const {//assuming there are no repetitions in the data
      ...
      // return ((id_taxi <  v.id_taxi) ||
      //         (id_taxi == v.id_taxi && pickup_time < v.pickup_time) ||
      //         (id_taxi == v.id_taxi && pickup_time == v.pickup_time && dropoff_time < v.dropoff_time));
    }
  };
  ...
  struct QueryIterator {
    ...
    inline const Trip *  operator->() { return *(this->iter); }
    inline const Trip *  operator->() const { return *(this->iter); }
    ...
    inline QueryIterator operator--(int) { this->iter--; return *this; }
    inline QueryIterator operator+(int dif) { return QueryIterator(this->iter+dif); }
    inline ptrdiff_t     operator-(const QueryIterator &it) const { return this->iter-it.iter;}
