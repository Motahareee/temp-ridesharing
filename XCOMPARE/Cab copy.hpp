...
//}
  ...
  //if (this->stops.front().passengers==0) {
    ...
    if (d<0 || d+dis>MAX_DELAY_DISTANCE-(trip->pickup_wait*DEFAULT_SPEED) || d+dis<equ_walk_distance)
      return -1 ;
  ...
  for (StopList::iterator it=this->stops.begin(); it!=this->stops.end() && dis<MAX_DELAY_DISTANCE; ++it, ++order) {
    ...
    //else if (it->wait_stop!=-1 && it->occ+trip->trip->passengers<=this->capacity) {
      ...
      if ((d>=0) && (dis+d<=MAX_DELAY_DISTANCE-(trip->pickup_wait*DEFAULT_SPEED))) {
      ...
      //if ((d>=0) && (dis+d<=MAX_DELAY_DISTANCE)) {
        ...
        int cost = distance + d;
	      if (cost<min_cost) {
	        min_cost = cost;
	        pick_dist = d;
          pick_total_dis = dis+d ; //changed this
          so->pick = order;
	        so->drop = so->pick;
        }
      }
                          ...
                          int &cab_wait, const ServingOrder &so)
{
  ...
  if(this->stops.front().passengers == 0){
    ...
    if(this->pathDistance.size()){
      ...
      int distance = city->findShortestDistance(prev, pick_loc);
      distance+=dis ;
    }
    else{
      distance = city->findShortestDistance(this->cur_pos, pick_loc);
      this->dist_driven = 0 ;
    }
    ...
    //this->stops.push_back(Stop(0,drop_loc, -trip->trip->passengers, trip->distance, 0, trip));
  ...
  if (it==this->stops.end()) {
    ...
    int distance = 0 ;
    it-- ;
    distance = city->findShortestDistanceReverse(it->intersection, pick_loc);
    //int occ__ = it->occ +  ;
    it++;
