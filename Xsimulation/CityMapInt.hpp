...
public:
  ...
  CityMap(const char * filename): GRID_DEGREE(0.001)
  {
    ...
    //for(int i=0 ; i<this->poolNodes[4169].size() ; i++){
    //  cout<<this->poolNodes[4169][i].intersection<<", "<<this->poolNodes[4169][i].walk_distance<<endl ;
    //}
  }
...
inline void setIntersection(int idx, const Location &L)
  ...
  void loadFromText(const char *filename)
  {
    ...
    for (int i=0; i<nE; i++) {
      ...
      this->setStreet(i, Street(idx[0], idx[1]), weight*0.0006);
    }
    fclose(fi);
  }
