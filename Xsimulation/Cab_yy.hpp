...
inline int Cab::update_cab(CityMap *city, int time, stack<int> &rebalancing_set)
{
  ...
  //  cout<<"Front stop distance"<< this->stops.front().distance<<endl ;
...
//}
  ...
  //cout<<"cost to share ****"<<this->pathIntersection.size()<<endl;
    ...
    //else if (it->wait_stop!=-1 && it->occ+trip->trip->passengers<=this->capacity) {
      ...
      if ((d>=0) && (dis+d  && dis+d<=MAX_DELAY_DISTANCE)) {
        ...
        //if (d>=0 && (d+it->dist_acc+drop_cost-distance)<=MAX_EXTRA_DISTANCE-it->max_extra) {
          /*
          cout<<"prev"<<it->intersection<<endl;
          cout<<"HEREEEE "<<d <<endl;
          cout<<"HEREEEE "<<it->dist_acc<<endl;
          cout<<"HEREEEE "<<drop_cost<<endl;
          cout<<"HEREEEE "<<distance<<endl;
          cout<<"HEREEEE "<<MAX_EXTRA_DISTANCE<<endl;
          cout<<(d>=0)<<endl;
          cout<<((d+it->dist_acc+drop_cost - distance)<=MAX_EXTRA_DISTANCE)<<endl;
          cout<<d+it->dist_acc+drop_cost - distance<<" "<<MAX_EXTRA_DISTANCE<<endl;
        */
...
{
  //cout<<"add_trip  ServingOrder "<<so.pick<<" , "<<so.drop<<endl ;
...
//t = this->stops.begin();
  ...
  // Now inserting the trip. If this is the only trip (or at the end of list)
  ...
  if (it==this->stops.end()) {
    ...
    if(this->stops.front().passengers==0){ 
      ...
      distance = city->findShortestDistanceReverse(this->cur_pos, pick_loc);
      //trip->dist_delay = 0 ;
    }
    else{
      it-- ;
      distance = city->findShortestDistance(it->intersection, pick_loc);
      it++;
      //trip->dist_delay = distance ;
    }
    this->stops.push_back(Stop(0, pick_loc, trip->trip->passengers, distance, trip->trip->passengers, trip));
    this->stops.push_back(Stop(0, drop_loc, -trip->trip->passengers, trip->distance, trip->trip->passengers, trip));
    trip->dist_extra = 0; // should be changed
