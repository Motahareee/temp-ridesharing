...
public:
  ...
  struct Stop {
    ...
    Stop(): intersection(-1), passengers(0), distance(0), max_extra(0), max_delay(0), dist_acc(0), occ(0), trip(0) {}
    Stop(int i, int p, int d, int o, CabTrip *t): intersection(i), passengers(p), distance(d), max_extra(0), max_delay(0), 
...
public:
  ...
  virtual int   cost_to_share(CityMap *city, const CabTrip *trip, ServingOrder *so, int pick_loc, int drop_loc, int walk_wait);
  virtual void  add_trip(CityMap *city, CabTrip *trip, int pick_loc, int drop_loc, const ServingOrder &order);
...
inline int Cab::update_cab(CityMap *city, int time)
{
  ...
  if (time_passed<=0 && this->cur_pos>0)
    return this->cur_pos;
...
inline void Cab::gen_stop(CityMap *city)
{
  // this->cur_pos = -1; return; // if we want cabs to have teleporting capability
  if (this->cur_pos<0) {
    // if (this->id>11000) // jfk
    //   this->cur_pos = 10844;
    // else if (this->id>10500) // lga
    //   this->cur_pos = 10831;
    // else
  ...
  if (distance>0) {
    this->stops.push_front(Stop(location, 0, distance, 0, NULL));
...
inline int Cab::cost_to_share(CityMap *city, const CabTrip *trip, ServingOrder *so,int pick_loc, int drop_loc, int walk_time)
{
  ...
  int prev = this->pathIntersection.at(this->cur_index);

  int dis = this->pathDistance.at(this->cur_index)-this->dist_driven;
  ...
  for (StopList::iterator it=this->stops.begin(); it!=this->stops.end() && equ_walk_distance<dis && dis<MAX_DELAY_DISTANCE; ++it, ++order) {
    ...
    // check if the passengers would fit in the cab
      ...
      if ((d>=0) && (equ_walk_distance<=dis+d  && dis+d<=MAX_DELAY_DISTANCE)) {
        int cost = d;
        d = city->findShortestDistance(pick_loc, it->intersection);
        if (d>=0 && (d+it->dist_acc+drop_cost-distance)<=MAX_EXTRA_DISTANCE) {
          cost += d-dist_left;
          if ((cost+it->max_extra)<=MAX_EXTRA_DISTANCE && equ_walk_distance<=(cost+it->max_delay) && (cost+it->max_delay)<=MAX_DELAY_DISTANCE) {
            cost += drop_cost;
            if (cost<min_cost) {
              min_cost = cost;
              pick_dist = d;
...
{
  //printf(stderr, "in find_shareable_stop\n");
  int vacancy;
...
inline void Cab::add_trip(CityMap *city, CabTrip *trip, int pick_loc, int drop_loc, const ServingOrder &so)
{
  //cout<<"ABC 1"<<endl ;  
  StopList::iterator it = this->stops.begin();
  //cout<<"CHECK 1 "<<stops.size()<<" "<<(it == this->stops.begin())<<endl ;

  std::advance(it, so.pick);
  StopList::iterator drop_it = it;
  if (so.drop<this->stops.size()){
  	//cout<<"MOTIIIJOON -1 "<<so.pick<<endl ;
    std::advance(drop_it, so.drop-so.pick);
  }
  else{
  	//cout<<"MOTIIIIJOON - 2 "<<so.pick<<" "<<so.drop<<endl ;
    drop_it = this->stops.end();
  }
  int occ_pick=-1, occ_drop=-1;
  //cout<<"CHECK 2 "<<stops.size()<<" "<<(it == this->stops.begin())<<endl ;
  StopList::iterator stop = it;
  //cout<<" compare "<<(stop==stops.begin())<<endl ;
  ...
  //cout<<"CHECK 4 "<<stops.size()<<" "<<(stop == this->stops.begin())<<endl ;
  ...
  // shares based on pickup and dropoff.
  {
    ...
    //cout<<"come after!!"<<endl ;
    //cout<<"CHECK 5 "<<stops.size()<<" "<<(stop == this->stops.begin())<<endl ;
    //cout<< " extra :: "<<(stop==stops.end())<<" "<<(stops.begin() == stops.end()) << " "<< stops.size() << endl ;
    //cout<<"CAB9"<<endl ;
  ...
  if (so.pick == so.drop || it->passengers == 0) {
    occ_drop = occ_pick;
  ...
  if (it==this->stops.end()) {
    this->stops.push_back(Stop(pick_loc, trip->trip->passengers, 0.f, trip->trip->passengers, trip));
    this->stops.push_back(Stop(drop_loc, -trip->trip->passengers, trip->distance, trip->trip->passengers, trip));
    trip->dist_extra = 0;
  ...
  // otherwise, try to put it in between two other stops
    ...
    if (it==this->stops.begin()) {
      ...
      const Stop stop(pick_loc, trip->trip->passengers, distance, occ_pick, trip);
      if (!roaming) {
    ...
    // otherwise, it wouldn't affect our current path, we just need to insert the stop
      ...
      this->stops.insert(it, Stop(pick_loc, trip->trip->passengers, distance, occ_pick, trip));
      dist_extra += distance;
    }
    ...
    this->stops.insert(drop_it, Stop(drop_loc, -trip->trip->passengers, distance, occ_drop, trip));
