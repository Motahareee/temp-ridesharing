...
public:
    ...
    for (int i=0;i<this->params.num_trips;i++){
      if(this->params.trip[i].served){
        ...
        if(this->params.trip[i].dist_delay > MAX_DELAY_DISTANCE){
          cout<<"NOOO !!!! Max Delay Exceed"<<endl ;
        }     
  ...
  void updateCost(CabTrip *cab_trip, int start, int end, Solution *local)
  {
    ...
    for (int c=start; c<end; ++c) {
      ...
      //Check if the cab can reach the passenger in best case
      int src, dis ;
      if(!this->params.cabs[c]->pathIntersection.empty()){
        src = this->params.cabs[c]->pathIntersection.at(this->params.cabs[c]->cur_index) ;
        dis = this->params.cabs[c]->pathDistance.at(this->params.cabs[c]->cur_index)-this->params.cabs[c]->dist_driven;
      }
      else{
        src = this->params.cabs[c]->cur_pos ;
        dis=0 ;
      }
	...
	if(this->params.cabs[c]->cur_pos >= 0 && dis + this->params.city->findShortestDistance(src,pick) > MAX_DELAY_DISTANCE){
           continue ;
	}
  ...
  void applySolution() {
    cout<<"req_time "<<this->params.trip->trip->pickup_time<<endl;
    cout<<"req_pickloc "<<this->params.trip->pick_loc<<endl;
    cout<<"req_matched "<<this->solution.X.best_pick<<endl;
   
    if (this->solution.min_cab_num!=-1) {
      this->params.trip->distance = this->params.city->findShortestDistance(this->solution.X.best_pick, this->solution.X.best_drop) +  PICKUP_DELAY ; 
